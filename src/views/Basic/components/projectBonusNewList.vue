<template>
  <el-dialog :visible="dialogShow" center @close="cancel" append-to-body>
    <el-select v-model="projectID" default-first-option filterable clearable :placeholder="$t('__plzChoice')" style="display:block">
      <el-option v-for="item in ddlPBList" :key="item.ID" :label="item.ID+' '+item.Value" :value="item.ID">
